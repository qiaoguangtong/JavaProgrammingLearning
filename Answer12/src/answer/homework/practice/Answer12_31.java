...
public class Answer12_31 {
	...
	public static void main(String[] args) {
		...
		if (gender == 'M') {
			...
			if (index >= 0)
				System.out.println("Boy name " + name + " is ranked #" + (index + 1) + " in year " + year);
			else
				System.out.println("Boy name " + name + " is not ranked in year " + year);
		} else {
			int index = search(girlNames, year, name);
			if (index >= 0)
				System.out.println("Girl name " + name + " is ranked #" + (index + 1) + " in year " + year);
			else
